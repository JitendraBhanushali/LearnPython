...
$(".submitform,.validate-field").click(function () {
...
function showTableData() {
    ...
    local_user.forEach(item =>
        ...
        <td>${item.states}</td>
        <td>${item.hobbies}</td>
        <td>${`<button onClick='onDelete(this,"${item.email}")'>Delete</button`}</td>    
...
function onDelete(td, email) {
    // console.log('email ::'+email);
    if (confirm("Do you want to delete this Record?")) {
        row = td.parentElement.parentElement;
        // console.log(row);
        document.getElementById('tbl').deleteRow(row.rowIndex);
        deleteLocalStorageData(email);
    }
    clearForm();
}

//  Delete data from the local storage
function deleteLocalStorageData(email) {

    var user_records = new Array();
    user_records = JSON.parse(localStorage.getItem("users")) ? JSON.parse(localStorage.getItem("users")) : [];
    user_records.forEach(function (arrayItem) {
        if (arrayItem.email == email) {
            //  console.log(user_records.indexOf(arrayItem));
            user_records.splice(user_records.indexOf(arrayItem), 1);
        }
    });
    localStorage.setItem("users", JSON.stringify(user_records));
    showTableData();
...
$(document).ready(
    function () {
        ...
        $('.validate-field').on('change', function (event) {
            $("#validation").valid();
        });
